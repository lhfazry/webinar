export const sendConfirmationEmail = async (to: string, name: string) => {
    ...
    const senderEmail = import.meta.env.VITE_MAILTRAP_SENDER_EMAIL || "cs@rumahcoding.co.id";

    if (!token) {
        console.error("Mailtrap API token is missing.");
        return;
    }
    ...
    try {
        const response = await fetch(import.meta.env.VITE_MAILTRAP_ENDPOINT || "https://send.api.mailtrap.io/api/send", {
            method: "POST",
            headers: {
